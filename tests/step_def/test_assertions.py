...
@then(parsers.parse('I expect the response body of "{request_type}" should contain "{validate_input}"'))
def validate_response_body(context, request_type, validate_input):
    if request_type == "POST":
        assert_that(context["post_response"]).contains(validate_input)
    elif request_type == "GET":
        assert_that(context["get_response"]).contains(validate_input)
    elif request_type == "PUT":
        assert_that(context["put_response"]).contains(validate_input)
    elif request_type == "DELETE":
        assert_that(context["delete_response"]).contains(validate_input)
