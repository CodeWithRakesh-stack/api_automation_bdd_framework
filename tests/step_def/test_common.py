...
from tests.utils import TestRoutes
...
@pytest.fixture
@given(
    parsers.parse('I set the DELETE endpoint to "{endpoint_url}"')
)
@given(
    parsers.parse('I set the UPDATE endpoint to "{endpoint_url}"')
)
@given(parsers.parse('I set the GET endpoint to "{endpoint_url}"'))
@given(parsers.parse('I set the POST endpoint to "{endpoint_url}"'))
def set_api_endpoint(set_rest_api_base_url, endpoint_url):
    ...
    login_related_endpoint = test_routes.get_endpoint_by_partial_keyword(endpoint_url)
